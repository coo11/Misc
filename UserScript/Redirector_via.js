...
(async () => {
  ...
  // Weibo Client Switch
    ...
    switch (i) {
      ...
      case 3:
        return GM_registerMenuCommand("Open Weibo URL", () =>
    ...
    function getInfoByOid(oid) {
      ...
      fetch(
        `https://h5.video.weibo.com/api/component?page=%2Fshow%2F1034%3A${oid}`,
        {
          headers: {
        ...
        for (let group of groupedList) {
          GM_fetch("https://api.e-hentai.org/api.php", {
            ...
            json?.gmetadata?.forEach(({ gid, token, title }) => {
              let e =
                needCheckedGalleries[`https://${hostname}/g/${gid}/${token}/`];
  ...
  // Danbooru Ehance
    ...
    document.addEventListener("click", event => {
      ...
      if (
        el.tagName === "A" &&
        (el.parentElement.classList.contains("post-score") ||
          el.parentElement.classList.contains("post-favcount"))
      ) {
        if (!el.focused) {
          event.preventDefault();
          el.focused = true;
    ...
    if (pathname.startsWith("/posts/")) {
      ...
      await wait(0); // Weird bug: https://danbooru.donmai.us/posts/7339343
  ...
  // SauceNAO
    ...
    document
      .querySelectorAll(
        "img[src*='static/patreon'], img[src*='static/btn_donate'], img[src*='static/bannersmall'], img[src*='static/yourimage270']"
      )
      .forEach(
        img =>
          (img.src =
